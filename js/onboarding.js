...
(function () {
  ...
  const STEPS = ['brush', 'palette', 'engine'];
  ...
  // 公开钩子：app.js 在 endStroke 末尾、switchPalette 内、switchEngine 内调用
  ...
  function init() {
    ...
    if (stored === null) {
      if (isExistingUser()) {
        stored = { brush: 'skipped_existing_user', palette: 'skipped_existing_user', engine: 'skipped_existing_user' };
        try { localStorage.setItem(STORAGE_KEY, JSON.stringify(stored)); } catch (_) {}
        gtagTrack('onboarding_skipped', { reason: 'existing_user' });
      } else {
        stored = { brush: 'pending', palette: 'pending', engine: 'pending' };
        try { localStorage.setItem(STORAGE_KEY, JSON.stringify(stored)); } catch (_) {}
