...
両エンジンはキャリブレーション済みで、多くの場面で視覚的にほぼ同じ結果が得られます。左上の **MB/KM** ボタンでいつでも切り替え可能。キャンバスはストローク履歴から自動再描画されます。

- **KM** —— **デフォルトエンジン**。独自実装。32³ LUT で RGB を 38 波長の反射率スペクトル（[spectral.js](https://github.com/rvanwijnen/spectral.js) MIT ライセンスのデータ）にマッピングし、スペクトル空間で Kubelka-Munk 公式を適用。GPL v3。**計算精度が高く、顔料 RGB に敏感** —— 顔料 RGB のわずかな違いが目に見える混色の差として現れ、本物の絵具が細かな変化に反応する感覚に近いです。アルゴリズムの特性上、青+黄の組み合わせで緑が出やすい傾向があります。
- **Mixbox (MB)** —— [Mixbox](https://scrtwpns.com/mixbox/) の LUT ベースアルゴリズム、CC BY-NC 4.0。手動チューニングされたアンカー顔料 → やや粗いがより均一 —— 幅広い入力範囲で一貫した結果を出します。精度は KM より低いですが、安定性はより高い。

両エンジンの結果は近接しています —— デフォルトは KM（精度と感度重視）、より均一な感覚が欲しい場合や GPL ライセンスを避けたい場合は MB に切り替えてください。違いは 25–75% の濃度範囲と、特定の青+黄の複合色で最も顕著です。
